...
public struct QUICConfiguration: Sendable {
    ...
    ///
    /// The default implementation will generate a key per configuration on startup. To enable stateless resets
    /// across restarts, use `.defaultWithUserProvidedKey(:)` with a persistent key of 32 bytes or
    /// more, or provide a custom ``QUICStatelessResetTokenGenerator`` implementation.
    public var statelessResetTokenGenerator: any QUICStatelessResetTokenGenerator
    ...
    ///     - statelessResetTokenGenerator: The generator for stateless reset tokens.
    public static func server(
        ...
        statelessResetTokenGenerator: any QUICStatelessResetTokenGenerator = .defaultWithAutoGeneratedKey
    ) -> Self {
        self.init(
            role: .server,
